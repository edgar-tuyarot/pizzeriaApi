...
public class PedidoPizzaDAO {
            ...
            if (filas > 0) {
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        ...
                        return new PedidoPizza(idGenerado, pp.getPedidoId(), pp.getPizzaId(),pp.getCantidad());
                    }
                }
            }
    ...
    public PedidoPizza actualizar(PedidoPizza pedidoPizza){
        ...
        try (Connection conn = Database.connect();
            ...
            stmt.setInt(2, pedidoPizza.getPedidoId());
            stmt.setInt(3, pedidoPizza.getPizzaId());
        ...
        try (Connection conn = Database.connect();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    pedidoPizza.setPedidoId(rs.getInt("pedido_id"));
                    pedidoPizza.setPizzaId(rs.getInt("cliente_id"));
